...
- Support all six commands and all sixteen agents as one atomic compatibility surface.
...
- Codex subagent definitions expose every agent using Codex-native metadata and tool names while referring to the canonical agent prompt content.
...
- Agents: all sixteen definitions currently under `agents/`.
...
The change is complete only when all six commands, all sixteen agents, manifests, and documentation pass validation together.
...
- All sixteen agents can be dispatched through Codex-native subagent support.
