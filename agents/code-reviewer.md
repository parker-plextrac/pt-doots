...
**Correctness findings still owe input provenance.** Hunting a bad-input path is the job; asserting it as a defect on an input you invented is not. Before promoting a finding whose trigger is a specific input value, name where that value came from: a real sample in the repo or import corpus, a ticket attachment, an existing fixture, or something seen in a log or DB row. If the honest answer is "I constructed it," either find a real instance or report it at `low` as a question. State the provenance inline, one clause. This is a separate axis from Phase 5: that check asks whether the code defuses the concern, this one asks whether the input occurs.
