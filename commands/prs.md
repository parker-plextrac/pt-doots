...
**Team:** loaded from a user overlay file (see Step 0b). Falls back to Example Teammate (`teammate-handle`, QA) if no overlay is set.
...
4. If no overlay file exists or no rows parse, fall back to:
   ```
   TEAM = [{ login: "teammate-handle", display_name: "Example Teammate (QA)" }]
   ```
...
**When to run it:**
- **Auto** when the PR touches repro-friendly code: the Python services (`product-services-export`, `product-services-mcp`), `a retired service`, or pure/isolatable logic (parsers, validators, mappers, date/uuid helpers, integration-worker parser code).
