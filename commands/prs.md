...
**QA engineer:** `teammate-handle` (Example Teammate — his PRs are prefixed with `QA-`)
...
- **QA — Example Teammate**: PRs where `user.login` is `teammate-handle`, **created within the last 40 days**
...
For each PR in **Your PRs**, **QA — Example Teammate**, and **IO Tickets (All Open)**, also fetch CI status:

```
mcp__github__get_pull_request_status(owner: "PlexTrac", repo: "{repo_name}", sha: "{head.sha}")
```

For each PR in **QA — Example Teammate** and **IO Tickets (All Open)**, also fetch review state to detect if Parker has reviewed:
...
If none: "No open PRs from a teammate."
